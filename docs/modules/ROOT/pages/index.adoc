...
=== Specify a version of an extension

Antora currently does not support specifying extension versions.
However, Quarkus Antora supports specifying those.
...
If you specify an AsciiDoc extension like in the example the above, then Quarkus Antora will do the following:

* Invoke `npm i asciidoctor-kroki@latest-0` before invoking `antora`
* Remove `@latest-0` from the effective `antora-playbook.yml` stored in `target/antora-playbook.yml` before passing it to `antora`.
